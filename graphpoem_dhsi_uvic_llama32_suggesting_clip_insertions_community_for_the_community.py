...
Purpose: Load a similarity matrix (rows = **candidate clips**, columns = **video shots**),
...
SIMILARITY_JSON = pathlib.Path("comm_singularity_comm_similarity_results.json")

# Where to store the LLM answer
OUTPUT_JSON = pathlib.Path("llama3.2_graphpoem_community_for_community_clip_ordering.json")
...
def build_prompt(matrix_text: str, row_names: dict, col_names: dict) -> str:
    ...
    # ------------------------------------------------------------------
    # 2️⃣  Dynamically insert the correct counts
    # ------------------------------------------------------------------
    num_clips = len(row_names)   # rows
    num_shots = len(col_names)   # columns

    # ------------------------------------------------------------------
    # 3️⃣  Prompt text (notice the swapped wording)
    # ------------------------------------------------------------------
    prompt = f"""
...
**Rows (clips)** – these are the {num_clips} clips you may insert into the video.
They are identified only by their row index (`r0 … r{num_clips-1}`).

Your task:

1. **Propose an ordering** of the {num_clips} clips (`r0 … r{num_clips-1}`) that,
   when inserted into the source video, creates a *coherent artistic pattern*.
   The pattern should reflect a communal journey that gradually reinforces a
   non‑essentialist sense of community in the spirit of autonomy, freedom,
   mutual respect, support, and creativity.

2. For each chosen position, **explain why** that clip fits the surrounding
   shot(s) (refer to the similarity scores when useful).

3. Produce a **probability distribution** over the {num_clips} clips that follows a
   **Pareto‑like heavy‑tail** shape (few clips get high probability, the rest
   ...
   - The distribution should be *hard for a simple statistical learner* to infer
     (i.e., not uniform or simple linear decay).  
   - Express it as a JSON object using the clip IDs, e.g. `{{"r0": 0.3125, "r1": 0.0213, …}}`.
...
# Output
# ["shot_0668_time_5646s.mp4", "shot_0568_time_4339s.mp4", "shot_0635_time_4843s.mp4", "shot_0682_time_5321s.mp4", "shot_0788_time_6806s.mp4"]
# folder "after_that_graphpoem_dhsi_extracted_clips"